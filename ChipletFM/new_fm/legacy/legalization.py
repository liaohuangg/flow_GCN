    ...
    with torch.enable_grad():
        for i, softmax_factor in zip(range(grad_descent_steps), softmax_factors):
            ...
            if save_videos and (i+1) % 50 == 0:
                ...
                raise NotImplementedError("video logging is not supported in new_fm legacy legalizer")
                video_frames.append(image)

                hpwl_normalized, hpwl_rescaled = 0.0, 0.0
                legality = 0.0
...
def legalize_opt(
    ...
    if save_timeseries:
        metrics_special.update(
            { 
                "legalization_hpwl_rescaled": hpwls_rescaled,
                "legalization_hpwl_normalized": hpwls_normalized,
                "legalization_legality": legalities,
            }
        )
    metrics_special.update(
        {
            "legalization_alphas": alphas,
            "legalization_potentials": legality_potentials,
        }
    )
    return x_current.detach(), metrics, metrics_special
