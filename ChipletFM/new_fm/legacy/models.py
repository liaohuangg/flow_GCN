...
class FlowMatchingModel(nn.Module):
    ...
    def __init__(
        ...
        alpha_init=0.0,
        alpha_lr=0.0,
        alpha_critical_factor=0.0,
        legality_potential_target=0.0,
        use_adam=False,
        guidance_mode="none",
        mask_key=None,
        use_mask_as_input=False,
        ...
        **kwargs,
        ...
        self.max_diffusion_steps = max_diffusion_steps
        self.device = device
        self.fm_t_min = fm_t_min

        self._lossfn = nn.MSELoss(reduction="mean")
        self._epsilon_dist = tfd.Normal(torch.tensor([0.0], device=device), torch.tensor([1.0], device=device))
        self._t_dist = tfd.Uniform(torch.tensor([fm_t_min], device=device), torch.tensor([1.0], device=device))
    ...
    def loss(self, x, cond, _):
        ...
        return self._loss(velocity_pred, velocity_target, mask), metrics
    ...
    def forward_samples(self, x, cond, intermediate_every=0):
        ...
        step_size = intermediate_every / self.max_diffusion_steps if intermediate_every else 1
        for t in torch.arange(0, 1 + (1e-9), step_size, device=x.device):
            z = self._epsilon_dist.sample(x.shape).squeeze(dim=-1)
            x_t = (1 - t) * x + t * z
            x_t = torch.where(mask, x, x_t) if mask is not None else x_t
            intermediates.append(x_t)
    ...
    def reverse_samples(self, B, x_in, cond, num_timesteps=-1, intermediate_every=0, mask_override=None):
        ...
        for step in range(num_timesteps, 0, -1):
            ...
            x = torch.clamp(x, -2, 2)
            if intermediate_every and ((num_timesteps - step + 1) % intermediate_every == 0):
                intermediates.append(x)
    ...
    def reverse_guidance_force(self, x_current, cond, t, mask=None):
        """
        Stateless SGD guidance for flow matching.

        In the FM sampler, the model state is the placement itself rather than a
        diffusion latent with an explicit x0 estimate, so the guidance operates
        directly in current-state space.
        """
        legality_softmax_factor = self.get_legality_softmax_factor(t)
        x_guided = x_current.detach().clone().requires_grad_(True)
        optimizer = torch.optim.SGD((x_guided,), lr=self.grad_descent_rate, momentum=0.0)
        for _ in range(self.grad_descent_steps):
            optimizer.zero_grad()
            h_legality = self.legality_guidance_weight * guidance.legality_guidance_potential(
                x_guided,
                cond,
                mask=mask,
                softmax_factor=legality_softmax_factor,
            )
            h_hpwl = self.hpwl_guidance_weight * guidance.hpwl_guidance_potential(x_guided, cond)
            h = h_legality + h_hpwl
            h.sum().backward()
            optimizer.step()
        return x_guided.detach() - x_current.detach()

    @torch.enable_grad()
    def reverse_guidance_opt_force(self, x_current, cond, t, mask=None):
        """
        Stateful adaptive guidance for flow matching, mirroring the constrained
        optimization variant used by ContinuousDiffusionModel.
        """
        legality_softmax_factor = self.get_legality_softmax_factor(t)
        x_guided = x_current.detach().clone().requires_grad_(True)
        if self.use_adam:
            optimizer_x = torch.optim.Adam((x_guided,), lr=self.grad_descent_rate, betas=(0.8, 0.99))
        else:
            optimizer_x = torch.optim.SGD((x_guided,), lr=self.grad_descent_rate, momentum=0.0)

        for _ in range(self.grad_descent_steps):
            optimizer_x.zero_grad()
            h_legality_raw = guidance.legality_guidance_potential(
                x_guided,
                cond,
                mask=mask,
                softmax_factor=legality_softmax_factor,
    ...
    def get_legality_softmax_factor(self, t):
        if t > self.legality_softmax_critical_factor:
            t_prime = (t - self.legality_softmax_critical_factor) / (1 - self.legality_softmax_critical_factor)
            legality_softmax_factor = self.legality_softmax_factor_max - t_prime * (
                self.legality_softmax_factor_max - self.legality_softmax_factor_min
            )
        else:
            return self.legality_softmax_factor_max
        return legality_softmax_factor
