...
def load_consolidated_graph_data(dataset_path, config, train_data_limit=None, val_data_limit=None):
    dataset = open_pickle(dataset_path)
    assert isinstance(dataset, dict), "consolidated graph dataset must be a dict"
    assert "train" in dataset and "val" in dataset, "consolidated graph dataset must contain train/val splits"
    ...
    if train_data_limit is None or train_data_limit == "none":
        train_data_limit = len(train_examples)
    if val_data_limit is None or val_data_limit == "none":
        val_data_limit = len(val_examples)
    assert train_data_limit <= len(train_examples) and val_data_limit <= len(val_examples), "data limits invalid"

    def prepare_split(examples, limit):
        output = []
        for idx, example in enumerate(examples[:limit]):
            cond = example["graph"]
            x = example["placement"]
            cond = load_and_parse_graph_object(cond)
            if "chip_size" in cond:
                if len(cond.chip_size) == 4:
                    chip_size = (cond.chip_size[2] - cond.chip_size[0], cond.chip_size[3] - cond.chip_size[1])
                    chip_offset = (cond.chip_size[0], cond.chip_size[1])
                else:
                    chip_size = (cond.chip_size[0], cond.chip_size[1])
                ...
                chip_offset = (0, 0)
            x, cond = preprocess_graph(x, cond, chip_size, scale, chip_offset=chip_offset)
            cond.file_idx = example.get("file_idx", idx)
            output.append((x, cond))
        return output

    train_set = prepare_split(train_examples, train_data_limit)
    val_set = prepare_split(val_examples, val_data_limit)
    return train_set, val_set
...
def load_and_parse_graph(path):
    ...
    return load_and_parse_graph_object(graph)

def load_and_parse_graph_object(graph):
